...
(function rjInstallHook() {
  ...
  /* Initialisation de l'état des pilotes une fois la grille construite.
     Le minuteur à zéro servait à attendre la fin du lancement : le registre
     s'exécute déjà au bon moment. */
  ...
  window.RJ_RACE_START_HOOKS.push({
    ...
    apres: function () {
      ...
      } catch (e) { console.warn("[RJ] Erreur init DriverStates:", e); }
    }
  });
  ...
  if (typeof window.updateLivePositions === "function" && !window._rjPositionFixInstalled) {
    ...
    /* Le bloc qui enveloppait ici updateLivePositions a été retiré.
       Mesuré : il n'était plus appelé depuis que le calcul des positions
       est assuré par un module unique (81-moteur-course), qui remplace la
       fonction au lieu de l'envelopper. Ce code restait chargé et lu sans
       jamais s'exécuter. */
  }
  ...
  if (!window._rjPenaltyFixInstalled) {
    ...
    /* Arrondi des pénalités à une décimale, une fois par tour : c'est le
       registre de tour (module 81) qui convient, et non une enveloppe sur
       le calcul des positions. */
    if (!Array.isArray(window.RJ_LAP_HOOKS)) window.RJ_LAP_HOOKS = [];
    if (!window.RJ_LAP_HOOKS.some(function (h) { return h && h.id === "04b-penalites"; })) {
