...
(function() {
  ...
  function wrapNavTo() {
    ...
    /* Ce module ne réagit qu'à l'entrée sur l'écran de course : il n'a pas
       à envelopper toute la navigation. Il s'inscrit au registre des
       réactions d'écran (module 88), qui ne l'appelle que pour S-race. */
    if (!Array.isArray(window.RJ_SCREEN_HOOKS)) window.RJ_SCREEN_HOOKS = [];
    if (!window.RJ_SCREEN_HOOKS.some(function (h) { return h && h.id === "09-race-header"; })) {
      window.RJ_SCREEN_HOOKS.push({
        id: "09-race-header",
        ecran: "S-race",
        apres: function () { setTimeout(updateRaceHeader, 30); }
      });
    }
    return true;
  }
