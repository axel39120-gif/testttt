...
function stIcon(s){
  ...
  function install(){
    ...
    if (!window.RJ_LEADERBOARD_HOOKS.some(function (h) { return h && h.id === "33-pitwall-telemetry"; })) {
      window.RJ_LEADERBOARD_HOOKS.push({
        id: "33-pitwall-telemetry",
        apres: function () {
          try { renderEnriched(); }
          catch (e) { console.warn('[33] rendu enrichi :', e && e.message); }
        }
      });
    }
