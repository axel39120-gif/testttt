...
(function rjInstallGraphicsHook() {
  ...
  window._rjGraphicsHookInstalled = true;
    ...
    /* Habillage graphique du classement : inscription au registre (89). */
    if (!Array.isArray(window.RJ_LEADERBOARD_HOOKS)) window.RJ_LEADERBOARD_HOOKS = [];
    if (!window.RJ_LEADERBOARD_HOOKS.some(function (h) { return h && h.id === "04h-graphics-overlay"; })) {
      window.RJ_LEADERBOARD_HOOKS.push({
        id: "04h-graphics-overlay",
        apres: function () {
