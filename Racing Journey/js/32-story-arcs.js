...
(function () {
  ...
  function wrapRefreshScreen(){
    ...
    /* Deux comportements distincts : prendre entièrement la main sur
       l'écran des arcs, et réagir au retour sur l'accueil. Le registre
       des réactions d'écran (module 88) sait exprimer les deux, sans
       envelopper toute la navigation. */
    if (!Array.isArray(window.RJ_SCREEN_HOOKS)) window.RJ_SCREEN_HOOKS = [];
    if (!window.RJ_SCREEN_HOOKS.some(function (h) { return h && h.id === "32-story-arcs"; })) {
      window.RJ_SCREEN_HOOKS.push({
        id: "32-story-arcs",
        ecran: ["S-arcs", "S-home"],
        remplace: function (id) {
          if (id !== "S-arcs") return false;
          ensureScreen(); renderArcsScreen();
          return true;
        },
        apres: function (id) {
