...
(function () {
  ...
  function oddsFromChoice(ch) {
    ...
    // 2) Format des événements en piste : chance.fail est la probabilité que
    //    le geste rate, chance.dnf celle qu'il tourne à la catastrophe,
    //    chance.penalty celle d'une sanction. Le champ « fail » était ignoré
    //    ici, si bien que des choix ouvertement risqués s'affichaient à
    //    100 % de réussite.
    if (ch.chance && (typeof ch.chance.dnf === "number" ||
                      typeof ch.chance.penalty === "number" ||
                      typeof ch.chance.fail === "number")) {
      var cri2 = pct(ch.chance.dnf || 0);
      var rate = pct(ch.chance.fail || ch.chance.penalty || 0);
      var ech2 = Math.max(0, rate - cri2);
    ...
    // 5) Choix sûr : aucune notion de risque dans sa définition. Ce n'est pas
    //    une raison pour ne rien afficher — un choix prudent réussit à tous
    //    les coups, et c'est précisément l'information qui permet de le
    //    comparer aux autres. Tous les choix affichent donc leurs chances.
    return { rea: 100, ech: 0, cri: 0 };
  }
  ...
  function enhanceBtn(btn) {
    ...
    if (!o) return; // ne devrait plus arriver : tout choix affiche ses chances
