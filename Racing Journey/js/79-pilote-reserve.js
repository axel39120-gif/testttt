...
(function () {
  ...
  /* La persistance passe par le registre commun (module 87) : un seul
     module ouvre le fichier de sauvegarde, quel que soit le nombre de blocs
     à y écrire. Avant cela, chaque module refaisait sa propre lecture, son
     analyse et sa réécriture du même fichier. */
  function installPersistance() {
    if (!Array.isArray(window.RJ_SAVE_HOOKS)) window.RJ_SAVE_HOOKS = [];
    if (window.RJ_SAVE_HOOKS.some(function (h) { return h && h.cle === "rj79"; })) return;
    window.RJ_SAVE_HOOKS.push({
      cle: "rj79",
      ecrire: function () { return blocSauvegarde(); },
      lire: function (bloc, G) {
        G._reserveDeal = (bloc && bloc.deal) || null;
        G._reserveHistory = (bloc && bloc.history) || [];
        G._freeSeats = (bloc && bloc.seats) || [];
        G._reserveMeta = (bloc && bloc.meta) || null;
        rafraichirAccueil();
      }
    });
  }
