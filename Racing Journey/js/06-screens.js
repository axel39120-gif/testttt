...
p.keys.forEach(function(kv){var v;if(kv[0]==="physique")v=phyVal;else v=src[kv[0]]||0;/* Les statistiques de talent se cumulent par petits incréments décimaux (entraînement, progression), et la virgule flottante fait le reste : « 46.05 » devenait « 46.050000000000004 » à l'affichage. Une aptitude se lit en nombre entier — on arrondit à l'affichage, sans toucher à la valeur interne qui, elle, doit garder sa finesse pour la progression. */v=Math.round(v);var pct=Math.round(100*Math.max(0,(v-35))/(99-35));html+='<div style="margin-bottom:4px;last-child:margin-bottom:0"><div style="display:flex;justify-content:space-between;font-size:10px;line-height:1.2;margin-bottom:2px"><span style="color:var(--text2)">'+kv[1]+'</span><strong style="color:'+p.color+';font-family:var(--font-display);font-size:11px">'+v+'</strong></div><div style="height:3px;background:var(--bg3);border-radius:2px;overflow:hidden"><div style="width:'+pct+'%;height:100%;background:'+p.color+'"></div></div></div>'});
html+='</div>'});
html+='</div></div>'}
