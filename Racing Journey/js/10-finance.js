...
(function _rjFinanceModule() {
  ...
  /* Ces données étaient posées dans G juste avant la sauvegarde, en
     supposant que le cœur sérialiserait tout l'objet. VÉRIFIÉ : il n'en
     sérialise qu'une liste fermée, et _rjFinanceSave n'y figurait pas.
     Les achats, les temps de recharge et les effets d'investissement
     étaient donc PERDUS à chaque rechargement de partie. Le registre de
     persistance (module 87) les écrit réellement. */
  (function _rjPersistFinance() {
    if (!Array.isArray(window.RJ_SAVE_HOOKS)) window.RJ_SAVE_HOOKS = [];
    if (window.RJ_SAVE_HOOKS.some(function (h) { return h && h.cle === "finance"; })) return;
    window.RJ_SAVE_HOOKS.push({
      ...
      lire: function (bloc, G) {
        if (!bloc || !G) return;
        G._rjPurchased = bloc.purchased || {};
        G._rjCooldowns = bloc.cooldowns || {};
        G._rjInvestmentEffects = bloc.effects || [];
        G._rjMaterialReduction = bloc.materialReduction || 0;
        G._rjNextRaceBonus = bloc.nextRaceBonus || 0;
      }
    });
  })();
