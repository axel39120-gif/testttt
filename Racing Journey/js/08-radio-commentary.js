...
(function() {
  ...
  function _collectExternalSources() {
    ...
    for (var i = 0; i < log.length; i++) {
      ...
      /* Les arrêts des rivaux saturaient le direct : sur une course courte,
         cinq d'entre eux rentrent au même tour et le fil n'affichait plus
         que « untel entre aux stands ». Ce n'est pas une information pour
         le joueur — seul son propre arrêt le concerne, et les faits de
         course qui le touchent. On ne garde donc que le sien. */
      if (isPit) {
        var moi = "";
        try {
        ...
        if (!moi || e.text.indexOf(moi) < 0) continue;
      }
  ...
  function wrapRenderLiveLeaderboardForCommentary() {
    ...
    /* Enrichissement du classement : inscription au registre (module 89). */
    if (!Array.isArray(window.RJ_LEADERBOARD_HOOKS)) window.RJ_LEADERBOARD_HOOKS = [];
    if (!window.RJ_LEADERBOARD_HOOKS.some(function (h) { return h && h.id === "08-radio-commentary"; })) {
      window.RJ_LEADERBOARD_HOOKS.push({
        id: "08-radio-commentary",
        apres: function () {
  ...
  function wrapRunRaceLive() {
    ...
    /* Remise à zéro du fil radio au départ. Rien ici n'a besoin d'entourer
       le lancement : une inscription au registre suffit et libère
       runRaceLive d'une enveloppe. */
